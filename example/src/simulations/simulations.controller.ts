...
export class SimulationsController {
    ...
    @Post('query')
    @ApiOperation({ summary: 'Simulate a database query' })
    ...
    @Post('cache')
    @ApiOperation({ summary: 'Simulate a cache operation' })
    async testCache(@Body() body: { operation?: 'get' | 'set' | 'del' | 'clear' }): Promise<{ success: boolean }> {
    ...
    @Post('event')
    @ApiOperation({ summary: 'Simulate an event dispatch' })
    ...
    @Post('job')
    @ApiOperation({ summary: 'Simulate a background job' })
    async testJob(@Body() body: { status?: string }): Promise<{ success: boolean }> {
        ...
        const status = (body.status as any) || statuses[Math.floor(Math.random() * statuses.length)];
    ...
    @Post('mail')
    @ApiOperation({ summary: 'Simulate a sent/failed email' })
    ...
    @Post('schedule')
    @ApiOperation({ summary: 'Simulate a scheduled task run' })
    async testSchedule(@Body() body: { status?: string }): Promise<{ success: boolean }> {
        ...
        await this.collector.collect('schedule', {
            name: schedule.name,
            cron: schedule.cron,
            interval: (schedule as any).interval,
            status,
    ...
    @Post('redis')
    @ApiOperation({ summary: 'Simulate a Redis command' })
    ...
    @Post('model')
    @ApiOperation({ summary: 'Simulate an ORM/Model action' })
    async testModel(@Body() body: { action?: string }): Promise<{ success: boolean }> {
        const actions: Array<{ action: any; entity: string; data?: unknown; where?: unknown; recordCount?: number }> = [
    ...
    @Post('notification')
    @ApiOperation({ summary: 'Simulate a notification' })
    async testNotification(@Body() body: { type?: string }): Promise<{ success: boolean }> {
        const notifications: Array<{ type: any; recipient: string | string[]; title: string; message: string; status: 'sent' | 'failed'; error?: string }> = [
    ...
    @Post('view')
    @ApiOperation({ summary: 'Simulate a template view rendering' })
    async testView(@Body() body: { format?: string }): Promise<{ success: boolean }> {
        const views: Array<{ template: string; format: any; locals?: Record<string, unknown>; dataSize?: number; outputSize?: number }> = [
    ...
    @Post('command')
    @ApiOperation({ summary: 'Simulate a CLI command' })
    async testCommand(@Body() body: { status?: string }): Promise<{ success: boolean }> {
        ...
        const statuses: any[] = ['executing', 'completed', 'failed'];
        const statusMap: Record<string, any> = { running: 'executing', completed: 'completed', failed: 'failed' };
        const status: any = body.status ? (statusMap[body.status] || 'completed') : statuses[Math.floor(Math.random() * statuses.length)];
    ...
    @Post('gate')
    @ApiOperation({ summary: 'Simulate a security gate check' })
    ...
    @Post('batch')
    @ApiOperation({ summary: 'Simulate a batch process' })
    ...
    @Post('dump')
    @ApiOperation({ summary: 'Simulate a database dump/backup' })
    async testDump(@Body() body: { status?: string }): Promise<{ success: boolean }> {
        const dumps: Array<{ operation: any; format: any; source?: string; destination?: string; recordCount?: number; compressed?: boolean; encrypted?: boolean }> = [
    ...
    @Post('http-client')
    @ApiOperation({ summary: 'Simulate an outbound HTTP request' })
