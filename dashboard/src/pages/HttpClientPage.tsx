    ...
    () => [
      {
        ...
        render: (entry) => (
          <ClickableBadge
            onClick={(e) => {
              e.stopPropagation();
              addFilter('methods', entry.payload.method);
            }}
            className="font-mono"
          >
      ...
      },
      {
        ...
        width: '80px',
        render: (entry) => {
          // 0 is what a request that never got a response records, and "ERR" is
          // the label for it.
          const statusCode = entry.payload.statusCode ?? 0;

          return (
            <ClickableBadge
              onClick={(e) => {
                e.stopPropagation();
                addFilter('statuses', statusCode > 0 ? String(statusCode) : 'ERR');
              }}
            >
      ...
      },
      {
        ...
        width: '180px',
        render: (entry) => {
          const hostname = entry.payload.hostname;
          return hostname ? (
            <ClickableBadge
              onClick={(e) => {
                e.stopPropagation();
                addFilter('hostnames', hostname);
              }}
              className="font-mono"
            >
              {hostname}
