  ...
  const tableColumns: Column<ViewEntry>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        width: '100px',
        render: (entry) => {
          const format = entry.payload.format;
          return format ? (
            <ClickableBadge
              onClick={(e) => {
                e.stopPropagation();
                addFilter('formats', format);
              }}
              className="font-mono"
            >
              {format}
      ...
      },
      {
        ...
        align: 'center',
        render: (entry) =>
          entry.payload.cacheHit !== undefined ? (
  ...
  return (
    <div>
      ...
      <PageHeader
        ...
        filterControls={<RangeFilters route="views" />}
